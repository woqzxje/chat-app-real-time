  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-6 overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ''}`}>
      <div className='pb-6'>
        ...
        <div className='bg-[#282142] rounded-full flex items-center gap-3 py-4 px-5 mt-6'>
      ...
      <div className='flex flex-col gap-3'>
        {filteredUsers.map((user) => (
          <div onClick={() => { setSelectedUser(user); setUnseenMessages(prev => ({ ...prev, [user._id]: 0 })) }}
            key={user._id} className={`relative flex items-center gap-3 p-4 pl-5 rounded-xl cursor-pointer ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
            ...
            {unseenMessages && unseenMessages[user._id] > 0 && <p className='absolute top-4 right-4 bg-violet-500 text-sm rounded-full w-6 h-6 flex items-center justify-center'>{unseenMessages[user._id]}</p>}
          </div>
        ))}
      </div>
